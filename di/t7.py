"""
ABOUT THIS TEST:
- An async function has a sync generator as a dependency
"""
...
def bar2(a: int = 200):
    return a
def foobar(a: Annotated[int, Depends(bar)], b: Annotated[int, Depends(bar2)], c: Annotated[int, Depends(asyncbar)]):
# def foobar(a: Annotated[int, Depends(bar)], b: Annotated[int, Depends(bar2)]):
    return f"foobar<bar:{a}, bar2:{b}>"

@generic_di
async def foo(w: Annotated[int, Depends(asyncbar)], x, y: Annotated[int, Depends(bar)], z: Annotated[str, Depends(foobar)]):
...
def test(mode):
    ...
    return run_test(mode, "t7", runnee)
